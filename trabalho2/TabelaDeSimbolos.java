...
public class AnalisadorSemantico extends FAZEDORESBaseListener {
    ...
    */
    ...
    public void enterComandosSetup(FAZEDORESParser.ComandosSetupContext ctx)
    {
        ...
        {
            if(ctx.comandoSetup(i).getStart().equals("ativar"))
            {
                if(ctx.comandoSetup(i).pino(i).NUM_INT() != null)
                {
                    String numPino = ctx.comandoSetup(i).pino(i).getText();
                    ...
                    if(!tabelaAtual.existePino(num))
                    {
                        tabelaAtual.adicionarSimbolo(null, null, null, null, num);
                    }else
                    {
                        out.println("Esse pino já foi ativado anteriormente.");
                    }
                }else{
                    if(ctx.comandoSetup(i).pino(i).IDENT()!=null)
                    {
                        String nome = ctx.comandoSetup(i).pino(i).getText();
    ...
    {
        TabelaDeSimbolos tabelaDoSetup = pilhaDeTabelas.getTabela("ativar");
        TabelaDeSimbolos atual = pilhaDeTabelas.topo();
        
        for(int i = 0; i < ctx.cmdLoop().size(); i++)
        {
            String pino = ctx.cmdLoop(i).pino(i).getText();
            if(ctx.cmdLoop(i).pino(i).NUM_INT()!=null)
            {
                int numPino = Integer.parseInt(pino);
                if(!tabelaDoSetup.existePino(numPino))
                {
                    out.println("Para ser usado, um pino deve ser ativado");
                }
            }else{
                if(!tabelaDoSetup.existe(numPino))
                {
                    out.println("Para ser usado, um pino deve ser ativado");
                }
            }
